...
export async function searchForPlayers(query: string,page: number,size: number) {
    try {
        const response = await fetch(`http://localhost:8080${ENDPOINT}?searchQuery=${query}&page=${page}&size=${size}`, {
            method: "GET",
            headers: {
        ...
        const data = await response.json() as Footballer[];
        return {
            data,
            error: null
        }
       ...
       return {
        data: null,
        error: error?.message || "An unknown error occurred while searching for players"
       }
    }
